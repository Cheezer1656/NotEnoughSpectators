package cheezer.notenoughspectators.tunnel;
...
public class TunnelClient extends Thread {
    public static final String REMOTE_HOST = "bore.cheesedev.xyz";
    ...
    public TunnelClient(int port) throws Exception {
        ...
        remotePort = parser.parse(response).getAsJsonObject().get("Hello").getAsInt();
    ...
    public void run() {
        try {
            while (true) {
                ...
                String uuid = parser.parse(message).getAsJsonObject().get("Connection").getAsString();
